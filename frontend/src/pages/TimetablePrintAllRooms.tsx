...
import { getRoomTimetable, type TimetableGridEntry } from '../api/timetable'
...
export function TimetablePrintAllRooms() {
  ...
  const [grids, setGrids] = React.useState<
    Array<{ roomId: string; roomCode: string; entries: TimetableGridEntry[] }>
  >([])
  ...
  React.useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        const [s, r] = await Promise.all([listTimeSlots(), listRooms()])
        if (cancelled) return
        setSlots(s)

        const active = r.filter((x) => Boolean(x.is_active)).sort((a, b) => a.code.localeCompare(b.code))
        setRooms(active)
        ...
        const results = await mapWithConcurrency(
          ...
          6,
          async (room) => {
            const data = await getRoomTimetable(room.id, runId)
            return { roomId: room.id, roomCode: room.code, entries: data }
          },
          (done, total) => {
  ...
  React.useEffect(() => {
    ...
    if (grids.length !== rooms.length) return

    setAutoPrinted(true)
    window.setTimeout(() => window.print(), 250)
  }, [autoPrinted, error, grids.length, loading, rooms.length, runId])
    ...
    <div className="min-h-dvh bg-white text-slate-900">
      <div className="no-print border-b bg-white px-6 py-4">
        <div className="mx-auto flex max-w-[1400px] items-center justify-between gap-3">
          <div>
            <div className="text-sm font-semibold">Print All Rooms</div>
            <div className="mt-1 text-xs text-slate-500">Opens one print job with a page per room.</div>
          </div>
      ...
      <div className="mx-auto max-w-[1400px] px-6 py-6">
        <div className="mb-4">
          <div className="text-lg font-semibold">{programCode} · All Rooms</div>
        ...
        {!runId ? (
          ...
          <div className="rounded-xl border bg-slate-50 p-4 text-sm text-slate-700">No rooms found.</div>
        ) : (
