    ...
    *,
    ...
    for sec in section_list:
        ...
        for subj_id in subj_ids:
            ...
            if sessions <= 0:
                continue
            direct_required_by_section[sid] += int(sessions)
            subj_type = str(getattr(subj, "subject_type", "THEORY")).upper()
            if subj_type == "LAB":
                direct_required_by_room_type["LAB"] += int(sessions)
            else:
                direct_required_by_room_type["THEORY"] += int(sessions)

            tid = assigned_teacher_by_section_subject.get((sec_id, subj_id))
            if tid is not None:
                direct_required_by_teacher[str(tid)] += int(sessions)
