...
-- =========================================================
-- Seed: Supabase CSE Semester 6 data as Academic Year = 3
-- This adapts the earlier seed scripts to the new schema where
-- sections/subjects are scoped by academic_year_id.
--
-- Prereqs:
-- - Run 000_init_local_schema.sql
-- - Run 006_seed_academic_years.sql
-- - Ensure track_subjects exists (run 001_add_section_track_and_curriculum_tables.sql)
-- =========================================================

-- Resolve academic_year_id (Year 3)
do $$
declare
  v_year3_id uuid;
begin
  ...
  if v_year3_id is null then
    raise exception 'academic_years.year_number=3 not found (run 006_seed_academic_years.sql)';
  end if;
end $$;
...
insert into sections (program_id, academic_year_id, semester, code, name, strength, track, is_active)
select p.program_id, ay.academic_year_id, 6, seed.code, seed.name, 0, seed.track, true
from seed
cross join p
cross join ay
...
insert into subjects (program_id, academic_year_id, semester, code, name, subject_type, sessions_per_week, max_per_day, lab_block_size_slots, is_active)
select p.program_id, ay.academic_year_id, 6, seed.code, seed.name, seed.subject_type, seed.sessions_per_week, seed.max_per_day, seed.lab_block_size_slots, true
from seed
cross join p
cross join ay
on conflict (academic_year_id, code)
...
-- ---------- Teacher → Subject eligibility (Year 3) ----------
with
ay as (select id as academic_year_id from academic_years where year_number = 3),
p as (select id as program_id from programs where code = 'CSE'),
subj as (
